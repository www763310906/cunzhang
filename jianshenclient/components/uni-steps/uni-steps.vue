		...
		<view :class="'uni-steps-' + direction" class="uni-steps-items">
			<view v-for="(item,index) in options" :key="index" :class="{'uni-steps-process':index === active,'uni-steps-finish':index < active}"
			 class="uni-steps-item">
				<view :style="{color:index === active ? activeColor : ''}" class="uni-steps-item-title-container" style="display: flex;flex-direction: column;">
					<view>
						<view class="uni-steps-item-title" style="float: left;width: 75%;">{{ item.content }}</view>
						<view v-if="item.user" class="uni-steps-item-desc" style="float: right;">责任人:{{ item.user }}</view>
					</view>
					<view>
								<view  v-if="item.state ==1"  class="uni-steps-item-desc"  style="float: left;width: 75%;" >{{ item.reply }}</view>
						<view v-if="item.state ==1" class="uni-steps-item-desc" style="float: right;">回复人:{{ item.replyUser }}</view>
						<view v-if="item.createTime" class="uni-steps-item-desc" style="float: left;">{{ item.createTime }}</view>


						<button  v-if="item.state ==0"  class="mybtn-small" style="float: right;" @tap="btnclick(index)">回复</button>
