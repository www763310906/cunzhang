<template>
	<view class="content1">
		<view style="
		 position:fixed;
		 z-index: 100;right: 0;padding: 50rpx;bottom: 0;padding-bottom: 150rpx;">
			<image class="pic" src="../../static/img/plus.png" mode="aspectFill" @click="addrecord" />
		</view>  
...
<script>
	...
	export default {
		...
		methods: {
						...
						if (res.statusCode == 200) {
							if (res.data.code == 0) {
								...
								for (let i = 0, length = data.length; i < length; i++) {
									...
									if (data[i].pic != null) {
										pic = that.$serverPicUrl + data[i].frendPic;
									}

									that.list.push({
										id: data[i].id,
										nickname: data[i].frend,
										frendId: data[i].frendId,
										phone: data[i].frendPhone,
			...
			goDetail(e) {
				uni.navigateTo({
					url: './frenddetail?id=' + this.list[e].frendId
				});
